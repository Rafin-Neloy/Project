    ...
    if (!$flag) {
        ...
        if (file_exists($file)) {
            ...
            if (empty($existing_data)) {
                $arr1[] = array("Full Name"=>$full_name,"UserName"=>$User_Name,"Password"=>$User_Password,);
                $result = write(json_encode($arr1));
            } else {

                $existing_data_decode = json_decode($existing_data);

                array_push($existing_data_decode, array("Full Name"=>$full_name,"UserName"=>$User_Name,"Password"=>$User_Password));
